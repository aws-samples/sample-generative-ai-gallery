...
class ByocInswapperCodeBuildStack(Stack):
    def __init__(self, scope: Construct, id: str, repository: ecr.IRepository, **kwargs) -> None:
        super().__init__(scope, id, **kwargs)

        # Get the absolute path of the project root
        project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        inswapper_dir = os.path.join(project_root, "byoc", "inswapper")

        # Verify if the directory exists
        if not os.path.exists(inswapper_dir):
            raise ValueError(f"Directory not found: {inswapper_dir}")

        # Create an S3 bucket to store the Dockerfile and source files
        bucket = s3.Bucket(self, "ByocInswapperSourceBucket",
        ...
        s3_deployment = s3deploy.BucketDeployment(self, "DeployDockerfileAndSrc",
            sources=[s3deploy.Source.asset(inswapper_dir)],
            destination_bucket=bucket,
            destination_key_prefix="source")

        # Create a CodeBuild project
        self.project = codebuild.Project(self, "ByocInswapperBuildProject",
            project_name="ByocInswapperBuildProject",
            description="Build Docker image for BYOC Inswapper",
        ...
        CfnOutput(self, "InswapperCodeBuildProjectName", value=self.project.project_name)
